...
# -*- coding: utf-8 -*-
"""
Expense Analysis Dashboard V5 — 一體化本機伺服器
  ① 靜態檔案服務（開啟儀表板 HTML）
  ② 智慧 CORS Proxy（轉發至任意 AI API，目標 URL 由 ?url= query param 決定）

使用方式（只需一個指令）：
  python expense_cors_proxy.py            # 預設 Port 8080
  python expense_cors_proxy.py 9000       # 指定 Port

啟動後：
  1. 瀏覽器開啟 http://localhost:8080
  2. 點選 Expense_Analysis_Dashboard_V5.html
...
class ExpenseServerHandler(BaseHTTPRequestHandler):
    ...
    def _dir_listing(self, local_dir, url_path):
        ...
        rows = ''.join(rows)
        html = f'''<!DOCTYPE html><html><head><meta charset="utf-8"><title>費用分析儀表板 V5</title>
...
<small style="color:#555">Expense CORS Proxy · Port {PORT} · AI 面板開啟「CORS 代理」即可</small>
</body></html>'''
        data = html.encode()
        self.send_response(200)
